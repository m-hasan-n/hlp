...
function plot_arm_and_objects(hand_trajectory_xy, elbow_trajectory_xy,...
    shoulder_trajectory_xy,neck_trajectory_xy,object_change_pos,...
    object_rectangle_handles,object_id,trajectory_time_stamp,writerObj)
...
for ii = 1 :  size(hand_trajectory_xy,1)
    hold on
    %arm
    ...
    %objects
    current_position =  object_rectangle_handles(object_id).Position;
    Xnew = object_change_pos(ii,1) - current_position(3)/2;
    Ynew = object_change_pos(ii,2) - current_position(4)/2;
    updated_position = [Xnew Ynew current_position(3) current_position(4)];
    %         updated_position = [object_change_pos(jj,1) object_change_pos(jj,2) current_position(3) current_position(4)];
    object_rectangle_handles(object_id).Position = updated_position;
